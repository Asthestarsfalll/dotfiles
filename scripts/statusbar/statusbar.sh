...
refresh() {
    echo "refresh"
    _icons=''; _wifi=''; _cpu=''; _mem=''; _date=''; _vol=''; _bat=''; _music=''; _light=''; _tomato=''             # 重置所有模块的状态为空
    source $DWM/statusbar/temp                                           # 从 temp 文件中读取模块的状态
    xsetroot -name "$_icons$_light$_tomato$_music$_wifi$_cpu$_mem$_date$_vol$_bat"                   # 更新状态栏
}
...
cron() {
    ...
    while true; do
        ...
        [ $((i % 300)) -eq 0 ] && to=(${to[@]} bat light tomato)                      # 每 300秒 更新 bat
